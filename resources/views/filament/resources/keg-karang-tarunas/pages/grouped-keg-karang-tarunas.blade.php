<x-filament::page>
    ...
    @php
        $user = auth()->user();
        $canManage = $user && $user->role && ($user->role->isAdmin() || $user->role->isRT());
                                ...
                                <tbody>
                                    @forelse($g['rows'] as $r)
                                        <tr class="row" x-data="{ deleteOpen:false }">
                                            ...
                                            @if($canManage)
                                                <td class="td">
                                                    <a href="{{ \App\Filament\Resources\KegKarangTarunas\KegKarangTarunaResource::getUrl('edit', ['record' => $r['id']]) }}" class="btn">
                                                        {{ $r['edit_label'] ?? 'Edit' }}
                                                    </a>
                                                </td>
                                                <td class="td">
                                                    <button type="button" @click.prevent="deleteOpen = true" style="background:none;border:none;color:#dc2626;padding:0;cursor:pointer;display:inline-flex;align-items:center;gap:4px;" aria-label="Hapus kegiatan" title="Hapus">
                                                    ...
                                                    >
                                                        <div class="modal-card">
                                                            <div class="modal-header">
                                                                ...
                                                                <div class="modal-title">Hapus Kegiatan Karang Taruna</div>
                                                            </div>
