<x-filament::page>
    ...
    <div class="filters-summary">
        <div class="summary-card">
            <div class="summary-label">Total Pengajuan Surat</div>
            <div class="summary-value">{{ number_format($summary['total'] ?? 0) }}</div>
            <div class="summary-tooltip">
@php
    $detailTotal = $summary['detail']['total'] ?? [];
@endphp
@foreach($detailTotal as $kategori => $jumlah)
{{ $jumlah }} surat kategori {{ ucfirst($kategori) }}
@endforeach
            </div>
        </div>
        <div class="summary-card">
            <div class="summary-label">Surat Selesai</div>
            <div class="summary-value" style="color:#2ea7d6">{{ number_format($summary['selesai'] ?? 0) }}</div>
            <div class="summary-tooltip">
@php
    $detailSelesai = $summary['detail']['selesai'] ?? [];
@endphp
@foreach($detailSelesai as $kategori => $jumlah)
{{ $jumlah }} surat kategori {{ ucfirst($kategori) }}
@endforeach
            </div>
        </div>
        <div class="summary-card">
            <div class="summary-label">Surat Belum Selesai</div>
            <div class="summary-value" style="color:#2ea7d6">{{ number_format($summary['belum_selesai'] ?? 0) }}</div>
            <div class="summary-tooltip">
@php
    $detailBelum = $summary['detail']['belum_selesai'] ?? [];
@endphp
@foreach($detailBelum as $kategori => $jumlah)
{{ $jumlah }} surat kategori {{ ucfirst($kategori) }}
@endforeach
            </div>
        </div>
                    ...
                    <tbody>
                        @forelse($g['rows'] as $r)
                            <tr class="row" data-id="{{ $r['id'] }}">
                                ...
                                <td class="td">{{ $r['tgl_selesai'] ? \Illuminate\Support\Carbon::parse($r['tgl_selesai'])->format('Y-m-d') : '-' }}</td>
                                <td class="td">
                                    @php $s = strtolower((string) $r['status']); @endphp
                                    <span class="badge {{ $s === 'selesai' ? 'badge-success' : ($s === 'diajukan' ? 'badge-warning' : 'badge-gray') }}">{{ $r['status'] }}</span>
                                </td>
                                <td class="td"><a href="{{ \App\Filament\Resources\SuratKetWargas\SuratKetWargaResource::getUrl('view', ['record' => $r['id']]) }}" onclick="markViewed('suket','{{ $r['id'] }}')" style="color:#2ea7d6;">View</a></td>
                            </tr>
                        @empty
                            <tr class="row">
    ...
    <script>
        (function(){
            ...
            var set = getSet('viewed_suket_ids');
