    ...
    public function up(): void
    {
        ...
        Schema::table('tagihan_iuran_wargas', function (Blueprint $table) {
            if (! Schema::hasColumn('tagihan_iuran_wargas', 'PembayaranMidtrans_id')) {
                $table->string('PembayaranMidtrans_id', 40)->nullable()->after('status_id');
            }
        });
        $hasFk = DB::table('information_schema.KEY_COLUMN_USAGE')
            ...
            ->exists();
        if (! $hasFk && Schema::hasTable('pembayaran_midtrans') && Schema::hasColumn('pembayaran_midtrans', 'PembayaranMidtrans_id')) {
            Schema::table('tagihan_iuran_wargas', function (Blueprint $table) {
                $table->foreign('PembayaranMidtrans_id')
                    ->references('PembayaranMidtrans_id')
                    ->on('pembayaran_midtrans')
                    ->nullOnDelete();
            });
        }
    }
